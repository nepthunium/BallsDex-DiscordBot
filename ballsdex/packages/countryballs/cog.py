...
class CountryBallsSpawner(commands.Cog):
        ...
        *,
        ...
        if not ball:
            ...
            try:
                ...
                await ctx.send("No such planetball exists.")
                return
    ...
    async def giveball(self, ctx: commands.Context, ball: str, *users: discord.User):
        """
        Give a planetball to one or more users.

        If the planetball name has spaces, put it between quotes.
        Multiple users may be given afterwards (mention or ID)
        """
        if not users:
            await ctx.send("no users were providededded")
            return
        try:
            ball_model = await Ball.get(country__iexact=ball.lower())
        except DoesNotExist:
            await ctx.send("ball doesnt exssit!")
            return
        ...
        if len(users) > 1:
            await ctx.send(
                f'The "{ball_model.country}" planetball was given to {len(users)} users.'
            )
        else:
            await ctx.send(f'The "{ball_model.country}" planetball was given to {users[0]}.')
